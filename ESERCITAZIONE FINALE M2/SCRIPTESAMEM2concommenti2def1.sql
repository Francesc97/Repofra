...
join category cat on p.categoryid = cat.categoryid;     #con datediff >180 stabilisco che questi saranno veri(1), alrimenti falsi(0)
select * from sales;
...
where sonumber = 1;
select * from sales;
commit;    #Ho modificato dei campi di dateship poiché per i dati che avevo inserito, nessuno era vero, 1  (maggiore ai 180 giorni)
...
GROUP BY ProductID) AS SubVendite);   #la prima subquery mi permette di utilizzare l'ultimo anno censito come filtro
#La doppia subquery nell'having calcola la media delle somme annuali e le confronta con ogni somma per prodotto
...
VALUES 
  ...
  ('Camion dei Pompieri PlaySet', 3);      #ho dovuto inserire due prodotti per fare in modo che i due result set delle query precedenti non fossero vuoti
 ...
 from product p);
 select * from versionedenormalizzata;     #view creata con una subquery 
 ...
 from country c 
 join region r on c.regionid = r.regionid);
 select * from infogeografiche;       #view creata con inner join